#define Door_Security_Version "1.13"
...
//const char* mqtt_clientID = "AcademyUnit";
const char* mqtt_clientID = "MainBuildingUnit";
//const char* mqtt_clientID = "FLC";

//const char* mqtt_clientID = "Test Unit";

// ***** ----- NETWORK SETTINGS -----
//        Update these with values suitable for your network.
byte mac[]    = {  0xDE, 0xED, 0xBA, 0xFE, 0xFE, 0xEB };  // mac     = Give Unique MAC address. (change the end of the mac by adding 1 letter.)
...
//byte server[] = { acct.godinternational.org };   // server  = List Server Device IP
char server[] = "acct.godinternational.org";  
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (Debug == 1) {
       Serial.print("FC: Received at: [" + String(topic) + "]  | ");
...
boolean reconnectmqtt() {
  ...
  //    client.connect( mqtt_base, mqtt_user, mqtt_pass);
    if (Debug == 1) {
      Serial.println("LF: Reconn..." + int(reconnect));
    }
...
//                Serial.println(" - whole text : " + String(mqtt_text) );
                Serial.println("LF: conning to: " + String(mqtt_text));
      }
      mqtt_text = getcharArray(mqtt_base, mqtt_all_doors, mqtt_sub);
      if (Debug == 1) {
        Serial.println("LF:Resubs to " + String(mqtt_text));
      }
...
void setup() {
  ...
  while (!Ethernet.begin(mac)) {
    if (Debug == 1) {
      Serial.println("Eth Trying to Connect...");
    }
...
int time_func(String type, int ReSeT,  long previous, long interval, int door ) { // time_func( STRING type = "millis" or "mqtt", INT ReSeT [1 = reset millis or mqtt to current - else - normal time check] ,LONG previous[millis or mqtt], LONG interval(millis or mqtt))
  ...
  if (ReSeT == 1) {
    ...
    if (current - previous > interval) {
      if (Debug == 1) {
                Serial.println("Ft_f: current " + type + ": " + current);
                Serial.println("Ft_f: previous" + type + ": "  + previous);
                Serial.println("Ft_f: interval" + type + ": " + interval);
      }
      if (type == "mqtt") {
        previousMqtt = current;
      } else if (type == "millis") {
    ...
    for (int isopenloop = 0; isopenloop < mqtt_doors_rows; isopenloop++) {
      if (mqtt_doors[isopenloop][2] == 1) { //have isopen check with door array
        ...
        if (past_intervalOpen == 1) { //time is up relock door
          ...
          if (Debug == 1) {
                        Serial.println("LF: close door :" + mqtt_doors[isopenloop][0]);
          }
        }
      }
    }
...
//    digitalWrite(resetPin, LOW);
  ...
  if (past_intervalMqtt == 1) {
    for (int doornum = 0; doornum < mqtt_doors_rows; doornum++) {
      ...
      if (Debug == 1) {
        Serial.println("LF past mqtt pub: " + String(mqtt_text));
      }
    }
...
void CloseDoor(int door) {
  mqtt_text = getcharArray(mqtt_base, String(mqtt_doors[door][0]), " ");
  if (Debug == 1) {
    Serial.print("Function CloseDoor: Closing relay" + String(mqtt_text));
  }
...
void OpenDoorMan(int door) {
  mqtt_text = getcharArray(mqtt_base, String(mqtt_doors[door][0]));
  if (Debug == 1) {
    Serial.println("Function OpenDoorMan: " + String(mqtt_text));
  }
