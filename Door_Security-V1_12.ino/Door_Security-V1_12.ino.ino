...
#define Door_Security_Version "1.12"
...
#define Debug 0 // 1 for Serial Print; 0 for no serial.
...
byte mac[]    = {  0xDE, 0xED, 0xBA, 0xFE, 0xFE, 0xEF };  // mac     = Give Unique MAC address. (change the end of the mac by adding 1 letter.)
...
//    }
    ...
    // ... and resubscribe
    client.subscribe("security/door/1");
  }
...
//  while (!client.connect( mqtt_base, mqtt_user, mqtt_pass)) {
  while (!client.connect( mqtt_clientID )) {
    if (NotCONN == 5) {   //time is up relock door
      if (Debug == 1) {
